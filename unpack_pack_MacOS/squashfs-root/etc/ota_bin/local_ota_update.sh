...
if [ $? != 0 ] || [ "$new_main_os_version" = "" ]; then
    echo "failed to get ota released version" 1>&2
    local_on_ota_error
    exit 1
fi
...
ota_site_dir=$ota_site/ota_v$new_main_os_version
ota_site_version_ok=$ota_site/ota_v$new_main_os_version/ota_v$new_main_os_version.ok
