...
# 获得当前需要升级的kernel的设备名
local_get_current_kernel_dev_path()
{
    local str=
    local tmp=
    local name=kernel2

    str=`mtd_read_str ota`
    if [ $? != 0 ]; then
        return 1
    fi

    tmp=$(echo "$str" | grep "ota:kernel2")
    if [ "$tmp" == "" ]; then
        name=kernel
    fi

    tmp=`mtd_name_to_dev $name`
    if [ $? != 0 ]; then
        return 1
    fi
...
# 获得当前需要升级的rootfs的设备名
local_get_current_rootfs_dev_path()
{
    local str=
    local tmp=
    local name=rootfs2
