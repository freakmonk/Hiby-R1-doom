...
if [ $# != 3 ] ; then
    echo "usage: $0 ota/rootfs/file/name total_size /dev/mtdN" 1>&2
    exit 1
fi
...
flash_erase $ota_rootfs_dev 0 0
if [ $? != 0 ]; then
    set_write_failed "flash erase error $?"
    exit 1
fi

/etc/ota_bin/ota_img_data_provider.sh $ota_rootfs_name $ota_rootfs_size \
    | nandwrite -m -p --input-size=$ota_rootfs_size $ota_rootfs_dev -
